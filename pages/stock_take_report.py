...
month_end_query= """SELECT
    ...
    comments
FROM
    `classic-football-shirts.peoplevox.item_movement_history`
WHERE
    `to` = 'LDN.SHOP' -- Replace with specific shop code (e.g. MMI.SHOP(miami shop), NYC.SHOP (new york shop), MCR.SHOP (manchester shop), LA.SHOP (los angeles shop))
    AND comments = 'Stock registered using android device. ' -- some common comment to identify stock take movements are: "Picking Despatched Stock registered using android device",
    "Moved using android device",
    "Moved to temporary despatch location at pack bench",
    "Added through adjustment module",
    "Removed using android device",
    "Created",
    "Registered during stock take acceptance"

    AND date_timestamp >= '2025-06-11 00:00:00' 
    AND date_timestamp < '2025-07-11 00:00:00'-- Replace with specific date or range
"""
@st.cache_resource
...
st.session_state.prompt = "Run the stock take report for all the stores- london, manchester, new york, los angeles and miami on 11th June 2025"
...
with st.form(key="stock_query_form"):
    prompt = st.text_area(
...
if st.session_state.dataframe is not None:
    ...
    st.download_button(
        ...
        data=csv,
        file_name=f"stock-take-report-data-{pd.Timestamp.now().strftime('%Y%m%d')}.csv",
        mime="text/csv",
