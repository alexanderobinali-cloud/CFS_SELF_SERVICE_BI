...
month_end_query= """WITH base AS (
  SELECT
    ...
    t.b2b_pc_code,
    p.sizes AS product_size,
    t.b2c_pc_code,

    -- de-dupe marker: keep row with a size where possible
    ROW_NUMBER() OVER (
      PARTITION BY so.order_no, so.order_line_no
      ORDER BY CASE WHEN p.sizes IS NOT NULL THEN 1 ELSE 2 END
    ) AS rn
  FROM `classic-football-shirts`.snapshots.sales_orders_snapshot_last_month so
  JOIN `classic-football-shirts.reporting.sales_orders` t
    ON t.order_no = so.order_no
   AND t.product_id = so.product_id
  JOIN `classic-football-shirts.reference.product` p
    ON p.sku = so.product_sku
  WHERE
    so.order_date >= DATE_TRUNC(DATE_SUB(CURRENT_DATE(), INTERVAL 1 MONTH), MONTH)
    AND so.order_date < DATE_TRUNC(CURRENT_DATE(), MONTH)
)

SELECT
  ROW_NUMBER() OVER (ORDER BY order_date DESC, order_no) AS row_num,
  order_no,
  order_line_no,
  order_date,
  order_time,
  ...
  qty_invoiced,
  transaction_id,
  stripe_charges_amount,
  stripe_charges_amount_captured,
  stripe_charges_amount_refunded,
  stripe_fees,
  ...
  product_description,
  product_cost,
  product_printing_cost,
  product_price,
  product_source,
  product_buying_status,
  product_department,
  product_season,
  product_condition,
  unit_cost_price_gbp,
  unit_cost_price_local,
  total_cost_price_refunded_gbp,
  total_cost_price_refunded_local,
  total_cost_price_gbp,
  total_cost_price_local,
  unit_sale_price_gbp,
  unit_sale_price_local,
  total_sale_price_gbp,
  total_sale_price_local,
  discount_gbp,
  discount_local,
  giftcard_gbp,
  giftcard_local,
  product_refund_gbp,
  product_refund_local,
  discount_refund_gbp,
  discount_refund_local,
  unit_revenue_inc_vat_gbp,
  unit_revenue_inc_vat_local,
  total_revenue_inc_vat_gbp,
  total_revenue_inc_vat_local,
  unit_product_revenue_inc_vat_gbp,
  unit_product_revenue_inc_vat_local,
  total_product_revenue_inc_vat_gbp,
  total_product_revenue_inc_vat_local,
  unit_vat_gbp,
  unit_vat_local,
  total_vat_gbp,
  total_vat_local,
  unit_product_vat_gbp,
  unit_product_vat_local,
  total_product_vat_gbp,
  total_product_vat_local,
  unit_revenue_ex_vat_gbp,
  unit_revenue_ex_vat_local,
  total_revenue_ex_vat_gbp,
  total_revenue_ex_vat_local,
  unit_product_revenue_ex_vat_gbp,
  unit_product_revenue_ex_vat_local,
  total_product_revenue_ex_vat_gbp,
  total_product_revenue_ex_vat_local,
  total_margin_gbp,
  total_margin_local,
  product_margin_gbp,
  product_margin_local,
  shipping_charge_inc_vat_gbp,
  shipping_charge_inc_vat_local,
  shipping_charge_vat_gbp,
  shipping_charge_vat_local,
  shipping_charge_ex_vat_gbp,
  shipping_charge_ex_vat_local,
  shipping_refunded_gbp,
  shipping_refunded_local,
  free_shipping,
  duties_base_fee_gbp,
  duties_base_fee_local,
  duties_base_fee_tax_gbp,
  duties_base_fee_tax_local,
  duties_total_gbp,
  duties_total_local,
  total_refunded_gbp,
  total_refunded_local,
  is_giftcard,
  base_adjustment,
  pos_currency_code,
  pos_izettle,
...
# FROM `classic-football-shirts`.snapshots.sales_orders_snapshot_last_month so
#         JOIN `classic-football-shirts.reporting.sales_orders` t on t.order_no = so.order_no and t.product_id = so.product_id
#         JOIN `classic-football-shirts.reference.product` p on p.sku = so.product_sku
# WHERE
#     so.order_date >= DATE_TRUNC(DATE_SUB(CURRENT_DATE(), INTERVAL 1 MONTH), MONTH)
#     AND so.order_date < DATE_TRUNC(CURRENT_DATE(), MONTH)
# """
...
st.session_state.prompt = "Run the month end credit report for last month"
...
st.markdown(
    ...
    <div style="display: flex; align-items: center; gap: 15px; margin-bottom: 20px;">
        ...
        <div>
            <h1 style="margin-bottom: 0;">Month end credit report</h1>
            <p style="margin-top: 0; color: #888;">Download the monthly report</p>
        </div>
    </div>
    """,
...
with st.form(key="month_end_query_form"):
    prompt = st.text_area(
        "Describe the dataset you want to query:",
        value=st.session_state.prompt,
        height=100,
        key="prompt_input_month_end"
    )
...
if submit_query:
    ...
    if not prompt:
        ...
        with st.spinner("AI is generating your SQL query..."):
            st.session_state.sql_query['month_end'] = generate_sql_query(prompt)
...
if st.session_state.sql_query.get('month_end'):
    st.markdown("---")
    st.subheader("Generated BigQuery SQL")
    st.code(st.session_state.sql_query['month_end'] , language="sql")
    ...
    if st.button("📈 Generate Data from Query", use_container_width=True, type="primary"):
        ...
        with st.spinner("AI is simulating the query and generating data..."):
            st.session_state.dataframe = generate_data_from_query(st.session_state.sql_query['month_end'] )
...
if st.session_state.dataframe is not None:
    ...
    st.download_button(
        ...
        data=csv,
        file_name=f"month-end-report-data-{pd.Timestamp.now().strftime('%Y%m%d')}.csv",
        mime="text/csv",
